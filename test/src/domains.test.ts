...
describe("DomainsManager: backward compatibility and domain enabling", () => {
  ...
  describe("constructor", () => {
    ...
    it("enables all domains when undefined is passed as argument", () => {
      ...
      expect(enabledDomains.size).toBe(10);
      expect(Array.from(enabledDomains).sort()).toEqual(["advanced-security", "core", "extension-data", "pipelines", "repositories", "search", "test-plans", "wiki", "work", "work-items"]);
    });
  ...
  describe("string input handling", () => {
    ...
    it("Error and enables all domains when an invalid domain name string is passed", () => {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Error: Specified invalid domain 'invalid-domain'. Please specify exactly as available domains: advanced-security, pipelines, core, extension-data, repositories, search, test-plans, wiki, work, work-items"
      );
    });
  });
  ...
  describe("getAvailableDomains method", () => {
    it("returns the full list of available domains", () => {
      ...
      expect(availableDomains).toEqual(["advanced-security", "pipelines", "core", "extension-data", "repositories", "search", "test-plans", "wiki", "work", "work-items"]);
